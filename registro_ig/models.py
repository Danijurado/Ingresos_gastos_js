...
def select_ingreso():
    connectSelectIngreso=Conexion("SELECT sum(quantity) FROM movement WHERE quantity>0")
...
def select_egreso():  
    connectSelectEgreso=Conexion("SELECT sum(quantity) FROM movement WHERE quantity<0")
...
def update_by(id,registro):#['date','concept','quantity']
    connectUpdate=Conexion(f"UPDATE movement SET date=?,concept=?,quantity=? WHERE id={id}",registro)
